...
impl Problem for BstSearch {
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        for _ in 0..5 {
            ...
            tests.push(TestCase {
                data: Box::new(BstSearchTest { tree, target }),
            });
        }
        ...
        for _ in 0..5 {
            ...
            while vals.contains(&target) {
                target = rng.random_range(-150..=150);
            }
            tests.push(TestCase {
                data: Box::new(BstSearchTest { tree, target }),
            });
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10)
            .map(|_| {
                ...
                TestCase {
                    data: Box::new(BstMinimumTest { tree }),
                }
            })
            .collect()
    }
...
impl Problem for BstIsValid {
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        for _ in 0..5 {
            ...
            let tree = random_bst(&mut rng, size, -100, 100);
            tests.push(TestCase {
                data: Box::new(BstIsValidTest { tree }),
            });
        }
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10)
            .map(|_| {
                ...
                TestCase {
                    data: Box::new(BstRangeSumTest { tree, low, high }),
                }
            })
            .collect()
    }
...
fn ref_range_sum(tree: &[Option<i32>], low: i32, high: i32) -> i32 {
    ...
    fn helper(arena: &[TreeNode], node: Option<usize>, low: i32, high: i32) -> i32 {
        ...
        if val > low {
            sum += helper(arena, arena[idx].left, low, high);
        }
        if val < high {
            sum += helper(arena, arena[idx].right, low, high);
        }
        sum
    }
...
impl Problem for BstSortedArrayToBst {
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        let mut tests: Vec<TestCase> = (0..9)
            .map(|_| {
                ...
                nums.sort();
                TestCase {
                    data: Box::new(BstSortedArrayToBstTest { nums }),
                }
            })
            .collect();
        tests.push(TestCase {
            data: Box::new(BstSortedArrayToBstTest { nums: vec![0] }),
        });
        tests
    }
    ...
    fn run_solution(&self, test: &TestCase, _log: &mut OperationLog) -> SolutionResult {
        ...
        SolutionResult {
            ...
            actual: format!(
                "tree={actual:?}, inorder={in_vals:?}, valid={is_valid}, balanced={is_balanced}"
            ),
        }
    }
}
        ...
        (0..10)
            .map(|_| {
                ...
                let existing: std::collections::HashSet<i32> =
                    inorder(&arena, root).into_iter().collect();
                let mut val = rng.random_range(-120..=120);
                while existing.contains(&val) {
                    val = rng.random_range(-120..=120);
                }
                TestCase {
                    data: Box::new(BstInsertTest { tree, val }),
                }
            })
            .collect()
    }
...
impl Problem for BstDelete {
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        for _ in 0..7 {
            ...
            tests.push(TestCase {
                data: Box::new(BstDeleteTest { tree, val }),
            });
        }
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10)
            .map(|_| {
                ...
                TestCase {
                    data: Box::new(BstKthSmallestTest { tree, k }),
                }
            })
            .collect()
    }
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10)
            .map(|_| {
                ...
                let val = vals[rng.random_range(0..vals.len())];
                TestCase {
                    data: Box::new(BstInorderSuccessorTest { tree, val }),
                }
            })
            .collect()
    }
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10)
            .map(|_| {
                ...
                while qi == pi {
                    qi = rng.random_range(0..vals.len());
                }
                let p = vals[pi];
                let q = vals[qi];
                TestCase {
                    data: Box::new(BstLcaTest { tree, p, q }),
                }
            })
            .collect()
    }
...
impl Problem for BstRecover {
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10)
            .map(|_| {
                ...
                TestCase {
                    data: Box::new(BstRecoverTest { tree: broken_tree }),
                }
            })
            .collect()
    }
    ...
    fn run_solution(&self, test: &TestCase, _log: &mut OperationLog) -> SolutionResult {
        ...
        let orig_vals: std::collections::HashSet<i32> =
            inorder(&orig_arena, orig_root).into_iter().collect();
        let act_vals: std::collections::HashSet<i32> =
            inorder(&act_arena, act_root).into_iter().collect();
        ...
        SolutionResult {
            ...
            input_description: format!("tree={:?}", t.tree),
            expected: format!(
                "valid BST with same values, valid={}, vals_match={}",
                true, true
            ),
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10)
            .map(|_| {
                ...
                TestCase {
                    data: Box::new(BstCountNodesInRangeTest { tree, lo, hi }),
                }
            })
            .collect()
    }
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10)
            .map(|_| {
                ...
                TestCase {
                    data: Box::new(BstFromPreorderTest { preorder }),
                }
            })
            .collect()
    }
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10)
            .map(|_| {
                ...
                let tree = random_bst(&mut rng, size, -200, 200);
                TestCase {
                    data: Box::new(BstIteratorTest { tree }),
                }
            })
            .collect()
    }
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10)
            .map(|_| {
                ...
                TestCase {
                    data: Box::new(BstMergeTwoTest { tree1, tree2 }),
                }
            })
            .collect()
    }
