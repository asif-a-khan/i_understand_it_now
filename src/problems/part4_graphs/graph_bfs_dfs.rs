...
struct BfsOrder;
struct BfsOrderTest { n: usize, edges: Vec<(usize, usize)> }

impl Problem for BfsOrder {
    fn id(&self) -> &str { "graph_bfs_dfs_bfs_order" }
    fn name(&self) -> &str { "BFS Traversal Order" }
    fn topic(&self) -> &str { "graph_bfs_dfs" }
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10).map(|_| {
            ...
            TestCase { data: Box::new(BfsOrderTest { n, edges }) }
        }).collect()
    }
...
struct DfsOrder;
struct DfsOrderTest { n: usize, edges: Vec<(usize, usize)> }

impl Problem for DfsOrder {
    fn id(&self) -> &str { "graph_bfs_dfs_dfs_order" }
    fn name(&self) -> &str { "DFS Traversal Order" }
    fn topic(&self) -> &str { "graph_bfs_dfs" }
    fn difficulty(&self) -> Difficulty { Difficulty::Easy }
    fn description(&self) -> &str {
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10).map(|_| {
            ...
            let extra = rng.random_range(0..=n);
            let edges = random_connected_edges(&mut rng, n, extra);
            TestCase { data: Box::new(DfsOrderTest { n, edges }) }
        }).collect()
    }
...
struct IsConnected;
struct IsConnectedTest { n: usize, edges: Vec<(usize, usize)> }

impl Problem for IsConnected {
    fn id(&self) -> &str { "graph_bfs_dfs_is_connected" }
    fn name(&self) -> &str { "Is Graph Connected" }
    fn topic(&self) -> &str { "graph_bfs_dfs" }
    fn difficulty(&self) -> Difficulty { Difficulty::Easy }
    fn description(&self) -> &str {
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10).map(|i| {
            ...
            TestCase { data: Box::new(IsConnectedTest { n, edges }) }
        }).collect()
    }
...
struct SPTest { n: usize, edges: Vec<(usize, usize)>, src: usize, dst: usize }

impl Problem for ShortestPathUnweighted {
    fn id(&self) -> &str { "graph_bfs_dfs_shortest_path_unweighted" }
    fn name(&self) -> &str { "Shortest Path (Unweighted)" }
    fn topic(&self) -> &str { "graph_bfs_dfs" }
    fn difficulty(&self) -> Difficulty { Difficulty::Easy }
    fn description(&self) -> &str {
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10).map(|i| {
            ...
            TestCase { data: Box::new(SPTest { n, edges, src, dst }) }
        }).collect()
    }
...
struct FindPath;
struct FindPathTest { n: usize, edges: Vec<(usize, usize)>, src: usize, dst: usize }

impl Problem for FindPath {
    fn id(&self) -> &str { "graph_bfs_dfs_find_path" }
    fn name(&self) -> &str { "Find Any Path" }
    fn topic(&self) -> &str { "graph_bfs_dfs" }
    fn difficulty(&self) -> Difficulty { Difficulty::Easy }
    fn description(&self) -> &str {
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10).map(|i| {
            ...
            let src = rng.random_range(0..n);
            let dst = rng.random_range(0..n);
            TestCase { data: Box::new(FindPathTest { n, edges, src, dst }) }
        }).collect()
    }
    ...
    fn run_solution(&self, test: &TestCase, _log: &mut OperationLog) -> SolutionResult {
        ...
        // Validate the path
        let valid = if actual.is_empty() {
            false
        } else if actual[0] != t.src || actual[actual.len() - 1] != t.dst {
            false
        } else {
...
struct CloneGraph;
struct CloneGraphTest { n: usize, edges: Vec<(usize, usize)> }

impl Problem for CloneGraph {
    fn id(&self) -> &str { "graph_bfs_dfs_clone_graph" }
    fn name(&self) -> &str { "Clone Graph" }
    fn topic(&self) -> &str { "graph_bfs_dfs" }
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10).map(|_| {
            ...
            let extra = rng.random_range(0..=n);
            let edges = random_connected_edges(&mut rng, n, extra);
            TestCase { data: Box::new(CloneGraphTest { n, edges }) }
        }).collect()
    }

    fn run_solution(&self, test: &TestCase, _log: &mut OperationLog) -> SolutionResult {
        let t = test.data.downcast_ref::<CloneGraphTest>().unwrap();
        let mut expected_edges: Vec<(usize, usize)> = t.edges.iter()
        ...
        let mut actual_normalized: Vec<(usize, usize)> = actual_edges.iter()
            .map(|&(u, v)| (u.min(v), u.max(v)))
            .collect();
...
struct CourseScheduleTest { n: usize, prerequisites: Vec<(usize, usize)> }

impl Problem for CourseSchedule {
    fn id(&self) -> &str { "graph_bfs_dfs_course_schedule" }
    fn name(&self) -> &str { "Course Schedule" }
    fn topic(&self) -> &str { "graph_bfs_dfs" }
    fn difficulty(&self) -> Difficulty { Difficulty::Medium }
    fn description(&self) -> &str {
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10).map(|i| {
            ...
            TestCase { data: Box::new(CourseScheduleTest { n, prerequisites }) }
        }).collect()
    }
...
struct CourseScheduleIITest { n: usize, prerequisites: Vec<(usize, usize)> }

impl Problem for CourseScheduleII {
    fn id(&self) -> &str { "graph_bfs_dfs_course_schedule_ii" }
    fn name(&self) -> &str { "Course Schedule II (Topological Sort)" }
    fn topic(&self) -> &str { "graph_bfs_dfs" }
    fn difficulty(&self) -> Difficulty { Difficulty::Medium }
    fn description(&self) -> &str {
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10).map(|i| {
            ...
            TestCase { data: Box::new(CourseScheduleIITest { n, prerequisites }) }
        }).collect()
    }
    ...
    fn run_solution(&self, test: &TestCase, _log: &mut OperationLog) -> SolutionResult {
        ...
        let valid = if actual.len() != t.n {
            ...
            all_present.len() == t.n
                && t.prerequisites.iter().all(|&(a, b)| pos[b] < pos[a])
        };
...
struct IslandTest { grid: Vec<Vec<i32>> }

impl Problem for NumberOfIslands {
    fn id(&self) -> &str { "graph_bfs_dfs_number_of_islands" }
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10).map(|_| {
            ...
            let grid: Vec<Vec<i32>> = (0..rows)
                .map(|_| (0..cols).map(|_| if rng.random_range(0..3) == 0 { 0 } else { 1 }).collect())
                .collect();
            TestCase { data: Box::new(IslandTest { grid }) }
        }).collect()
    }
...
struct WordLadder;
struct WordLadderTest { begin: String, end: String, word_list: Vec<String> }

impl Problem for WordLadder {
    fn id(&self) -> &str { "graph_bfs_dfs_word_ladder" }
    fn name(&self) -> &str { "Word Ladder" }
    fn topic(&self) -> &str { "graph_bfs_dfs" }
    fn difficulty(&self) -> Difficulty { Difficulty::Medium }
    fn description(&self) -> &str {
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10).map(|_| {
            ...
            TestCase { data: Box::new(WordLadderTest { begin, end, word_list }) }
        }).collect()
    }
    ...
    fn run_solution(&self, test: &TestCase, _log: &mut OperationLog) -> SolutionResult {
        ...
        SolutionResult {
            is_correct: expected == actual,
            input_description: format!("begin={:?}, end={:?}, word_list={:?}", t.begin, t.end, t.word_list),
            expected: format!("{expected}"),
            actual: format!("{actual}"),
        }
    }
...
struct WordLadderII;
struct WordLadderIITest { begin: String, end: String, word_list: Vec<String> }

impl Problem for WordLadderII {
    fn id(&self) -> &str { "graph_bfs_dfs_word_ladder_ii" }
    fn name(&self) -> &str { "Word Ladder II (All Shortest Paths)" }
    fn topic(&self) -> &str { "graph_bfs_dfs" }
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10).map(|_| {
            ...
            let begin: String = (0..word_len)
                ...
                .collect();
            let end = word_list[rng.random_range(0..word_list.len())].clone();
            TestCase { data: Box::new(WordLadderIITest { begin, end, word_list }) }
        }).collect()
    }
            ...
            for c in b'a'..=b'z' {
                ...
                if let Some(&nd) = dist.get(&next_word) {
                    if nd == d + 1 {
                        ...
                        dfs(&next_word, end, dist, word_set, path, result);
                        path.pop();
                    }
                }
            }
...
struct SurroundedTest { board: Vec<Vec<char>> }

impl Problem for SurroundedRegions {
    fn id(&self) -> &str { "graph_bfs_dfs_surrounded_regions" }
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10).map(|_| {
            ...
            let board: Vec<Vec<char>> = (0..rows)
                ...
                .collect();
            TestCase { data: Box::new(SurroundedTest { board }) }
        }).collect()
    }
...
struct PacificAtlanticTest { heights: Vec<Vec<i32>> }

impl Problem for PacificAtlantic {
    fn id(&self) -> &str { "graph_bfs_dfs_pacific_atlantic" }
    fn name(&self) -> &str { "Pacific Atlantic Water Flow" }
    fn topic(&self) -> &str { "graph_bfs_dfs" }
    fn difficulty(&self) -> Difficulty { Difficulty::Hard }
    fn description(&self) -> &str {
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10).map(|_| {
            ...
            let heights: Vec<Vec<i32>> = (0..rows)
                ...
                .collect();
            TestCase { data: Box::new(PacificAtlanticTest { heights }) }
        }).collect()
    }
...
fn ref_pacific_atlantic(heights: &[Vec<i32>]) -> Vec<(usize, usize)> {
    ...
    for c in 0..cols { pacific_starts.push((0, c)); }
    for r in 1..rows { pacific_starts.push((r, 0)); }
    let pacific = bfs(heights, pacific_starts);

    // Atlantic: bottom row + right column
    let mut atlantic_starts = Vec::new();
    for c in 0..cols { atlantic_starts.push((rows - 1, c)); }
    for r in 0..rows - 1 { atlantic_starts.push((r, cols - 1)); }
...
struct AllPaths;
struct AllPathsTest { graph: Vec<Vec<usize>> }

impl Problem for AllPaths {
    fn id(&self) -> &str { "graph_bfs_dfs_all_paths" }
    fn name(&self) -> &str { "All Paths from Source to Target" }
    fn topic(&self) -> &str { "graph_bfs_dfs" }
    fn difficulty(&self) -> Difficulty { Difficulty::Hard }
    fn description(&self) -> &str {
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10).map(|_| {
            ...
            for row in &mut graph { row.sort(); }
            TestCase { data: Box::new(AllPathsTest { graph }) }
        }).collect()
    }
...
struct BinaryMatrixTest { grid: Vec<Vec<i32>> }

impl Problem for ShortestPathBinaryMatrix {
    fn id(&self) -> &str { "graph_bfs_dfs_shortest_path_binary_matrix" }
    fn name(&self) -> &str { "Shortest Path in Binary Matrix" }
    fn topic(&self) -> &str { "graph_bfs_dfs" }
    fn difficulty(&self) -> Difficulty { Difficulty::Hard }
    fn description(&self) -> &str {
    ...
    fn generate_tests(&self) -> Vec<TestCase> {
        ...
        (0..10).map(|_| {
            ...
            let mut grid: Vec<Vec<i32>> = (0..n)
                .map(|_| (0..n).map(|_| if rng.random_range(0..4) == 0 { 1 } else { 0 }).collect())
                .collect();
            ...
            TestCase { data: Box::new(BinaryMatrixTest { grid }) }
        }).collect()
    }
