...
impl Problem for BitsSingleNumber {
    ...
    fn run_solution(&self, test: &TestCase, log: &mut OperationLog) -> SolutionResult {
        ...
        let actual = solutions::single_number(&tracked_nums);
...
impl Problem for BitsMissingNumber {
    ...
    fn run_solution(&self, test: &TestCase, log: &mut OperationLog) -> SolutionResult {
        ...
        let shared_log = Rc::new(RefCell::new(OperationLog::new()));
        let tracked_nums = track_slice(&t.nums, shared_log.clone());
        let actual = solutions::missing_number(&tracked_nums);
        for op in shared_log.borrow().operations() {
            log.record(op.clone());
        }
        SolutionResult {
...
impl Problem for BitsSubsetsBitmask {
    ...
    fn run_solution(&self, test: &TestCase, log: &mut OperationLog) -> SolutionResult {
        ...
        let shared_log = Rc::new(RefCell::new(OperationLog::new()));
        let tracked_nums = track_slice(&t.nums, shared_log.clone());
        let mut actual = solutions::subsets_bitmask(&tracked_nums);
        for op in shared_log.borrow().operations() {
            log.record(op.clone());
        }
        actual.sort();
...
impl Problem for BitsTotalHammingDistance {
    ...
    fn run_solution(&self, test: &TestCase, log: &mut OperationLog) -> SolutionResult {
        ...
        let shared_log = Rc::new(RefCell::new(OperationLog::new()));
        let tracked_nums = track_slice(&t.nums, shared_log.clone());
        let actual = solutions::total_hamming_distance(&tracked_nums);
        for op in shared_log.borrow().operations() {
            log.record(op.clone());
        }
        SolutionResult {
...
impl Problem for BitsMaximumXor {
    ...
    fn run_solution(&self, test: &TestCase, log: &mut OperationLog) -> SolutionResult {
        ...
        let shared_log = Rc::new(RefCell::new(OperationLog::new()));
        let tracked_nums = track_slice(&t.nums, shared_log.clone());
        let actual = solutions::maximum_xor(&tracked_nums);
        for op in shared_log.borrow().operations() {
            log.record(op.clone());
        }
        SolutionResult {
...
impl Problem for BitsSingleNumberII {
    ...
    fn run_solution(&self, test: &TestCase, log: &mut OperationLog) -> SolutionResult {
        ...
        let shared_log = Rc::new(RefCell::new(OperationLog::new()));
        let tracked_nums = track_slice(&t.nums, shared_log.clone());
        let actual = solutions::single_number_ii(&tracked_nums);
        for op in shared_log.borrow().operations() {
            log.record(op.clone());
        }
        SolutionResult {
...
impl Problem for BitsSingleNumberIII {
    ...
    fn run_solution(&self, test: &TestCase, log: &mut OperationLog) -> SolutionResult {
        ...
        let shared_log = Rc::new(RefCell::new(OperationLog::new()));
        let tracked_nums = track_slice(&t.nums, shared_log.clone());
        let mut actual = solutions::single_number_iii(&tracked_nums);
        for op in shared_log.borrow().operations() {
            log.record(op.clone());
        }
        actual.sort();
...
impl Problem for BitsMaxAndPair {
    ...
    fn run_solution(&self, test: &TestCase, log: &mut OperationLog) -> SolutionResult {
        ...
        let shared_log = Rc::new(RefCell::new(OperationLog::new()));
        let tracked_nums = track_slice(&t.nums, shared_log.clone());
        let actual = solutions::max_and_pair(&tracked_nums);
        for op in shared_log.borrow().operations() {
            log.record(op.clone());
        }
        SolutionResult {
